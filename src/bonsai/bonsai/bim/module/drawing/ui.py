...
class BIM_PT_sheets(Panel):
    ...
    def draw(self, context):
        ...
        if active_sheet:
            ...
            # The operator resolves the drawing from the active sheet item itself (in invoke),
            # so we deliberately don't scan every IfcAnnotation here in draw() to set op.drawing
            # (that scan was a per-redraw performance drain).
