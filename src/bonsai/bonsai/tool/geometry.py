...
class Geometry(bonsai.core.tool.Geometry):
    ...
    def reimport_element_representations_batch(
        ...
        for obj, representation, apply_openings in items:
            ...
            # Types whose own representation is being reimported directly (as opposed to
            # types pulled in merely as the type of some occurrence being reimported).
            target_types: set[ifcopenshell.entity_instance] = set()
            ...
            for mapped_element in ifcopenshell.util.element.get_elements_by_representation(ifc_file, representation):
                if mapped_element.is_a("IfcTypeProduct"):
                    ...
                    target_types.add(mapped_element)
                else:
            ...
            # Record the base representation for types reimported directly, so the type
            # loop can render exactly that rep (e.g. a 'Reference' opening template)
            # instead of whatever get_representation() matches by context alone.
            for type_element in target_types:
                group["targets"][type_element] = representation
        ...
        for (context_id, apply_openings), group in groups.items():
            ...
            for type_element in element_types:
                if obj := tool.Ifc.get_object(type_element):
                    ...
                    # by context. When we are reimporting one of this type's own representations
                    # directly - such as switching to the Reference rep - render exactly that,
                    # otherwise the context lookup could return the wrong one. targets only holds
                    # a base representation for types reimported directly; for types pulled in as
                    # an occurrence's type it is None, so we fall back to the context lookup.
                    base_representation = targets.get(type_element)
                    type_representations = [
                        ifcopenshell.util.representation.resolve_representation(rm.MappedRepresentation)
                        for rm in (type_element.RepresentationMaps or [])
                    ]
                    if base_representation in type_representations:
                        type_representation = base_representation
                    else:
                        type_representation = ifcopenshell.util.representation.get_representation(
